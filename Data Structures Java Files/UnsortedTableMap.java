...
public class UnsortedTableMap<K, V> extends AbstractMap<K, V> {
	...
	private int findIndex(K key) {
		int n = table.size();
		for(int j=0; j<n; j++){   //Search table for index
	...
	public V remove(K key) {
		...
		if(j == -1){
			return null; //Index isn't there to remove
		}
		V answer = table.get(j).getValue();

		if(j != n-1){
			table.set(j, table.get(n-1));	//relocate last entry to 'hole' created by removed object
		}
		table.remove(n-1);	//remove last entry
		return answer;
	}
